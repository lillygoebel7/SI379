...
function displayGuessFeedback(guess) {
    ...
    const largeDiv = document.querySelector('#guesses');

    for (let i = 0; i<guess.length; i++) {
        const mySpan = document.createElement("span");
        mySpan.classList.add('letter');

        const letter = guess[i].toUpperCase();
        const correctLetter = correctAnswer[i].toUpperCase();
        if(letter === correctLetter) {
            mySpan.classList.add('correct');
            mySpan.innerText = letter;
            newDiv.append(mySpan);
        } else if(correctAnswer.toUpperCase().includes(letter)) {
            mySpan.classList.add('present');
            mySpan.innerText = letter;
            newDiv.append(mySpan);
        } else {
            mySpan.classList.add('absent');
            mySpan.innerText = letter;
            newDiv.append(mySpan);
        }
    }
    largeDiv.append(newDiv);
}
...
const inp = document.querySelector('#guess-inp');
const infoMessage = document.getElementById("info-message");
inp.addEventListener('keydown', (ev) => {
    if (ev.key == 'Enter') {
        const value = inp.value;
        if (value.length !== WORD_LENGTH) {
            infoMessage.innerText = "Your guess must be " + WORD_LENGTH + " letters long.";
        ...
        if (value != correctAnswer && value.length === WORD_LENGTH) {
            isValidWord(value, (result) => {
                if (result === true){
                    displayGuessFeedback(value);
                }
                else {
                    infoMessage.innerText = value + " is not a valid word.";
                    infoMessage.classList.remove('hidden');
                }
            });
            inp.value = "";
