...
getUMEventsWithImages((data) => {
    ...
    const nextImg = (index) => {
        clearInterval(myLastTimer); 
        curIndex = index; 
        setSelectedIndex(); 
        myLastTimer = setInterval(setSelectedIndex, 10000);
    };
    ...
    function setSelectedIndex() {
        ...
        title = eventsWithImages[curIndex].event_title;
        let selectedTitle = document.getElementById('selected-title');
        selectedTitle.textContent = title;

        let myLink = document.createElement('a');
        myLink = eventsWithImages[curIndex].permalink;
    ...
    setSelectedIndex();
    let myLastTimer = setInterval(setSelectedIndex, 10000);
